...
- **Multi-istanza (EdicolAI)**: un magazine per profilo Hermes, registry
  ...
  mai dal filename. Path sorgenti (derivati dall'istanza attiva) in
  `src/lib/content-dirs.ts`.
- Setup locale: `git submodule update --init` poi
  `git -C input/<slug>-daily lfs pull` per ogni submodule. Aggiornare
  i contenuti: `git submodule update --remote` (in CI lo fa il
  workflow content-sync ogni 30' + repository_dispatch, su tutti i
  submodule).
- Il contratto di ingestione per Hermes/EdicolAI è `docs/INGESTION.md`.
- Categorie e fonti sono **emergenti** dal corpus: niente enum fissi.
  Alias/normalizzazioni per istanza in `src/data/aliases/<slug>.ts`
  (`src/data/source-aliases.ts` seleziona quello dell'istanza attiva).
